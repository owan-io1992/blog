---
date: 2024-01-01T00:00:00  
draft: false
tags:
- certificate
title: "Certificate"
---

# 證書列表

以下是所有相關證書檔案：

| 證書名稱 | 下載連結 |
|---------|----------|
| AWS Certified Cloud Practitioner | [AWS_CCP.pdf](./files/AWS_CCP.pdf) |
| Cisco Certified Network Associate | [CCNA.pdf](./files/CCNA.pdf) |
| Certified Kubernetes Administrator | [CKA.pdf](./files/CKA.pdf) |
| Certified Kubernetes Application Developer | [CKAD.pdf](./files/CKAD.pdf) |
| ISO 27001 Information Security Management | [iso27001.PDF](./files/iso27001.pdf) |

## 證書說明

- **AWS_CCP.pdf**: AWS 雲端從業人員認證
- **CCNA.pdf**: Cisco 網路 associate 認證
- **CKA.pdf**: Kubernetes 管理員認證
- **CKAD.pdf**: Kubernetes 應用程式開發者認證
- **iso27001.PDF**: ISO 27001 資訊安全管理認證

所有證書檔案均可透過上表中的連結下載。

