...
| 資源 | 單位 | 範例 | 說明 |
| :--- | :--- | :--- | :--- |
| **CPU** | Cores | `1` (1 個核心)  `0.5` (半個核心)  `500m` (500 millicores) | `m` 代表 `millicore` (千分之一核心)。`1000m` 等於 `1` 個核心。`1m` 是最小單位。 |
| **Memory** | Bytes | `128974848` (bytes)  `129M` (megabytes)  `123Mi` (mebibytes) | 建議使用 2 的冪次方單位 (Ei, Pi, Ti, Gi, Mi, Ki) 以避免混淆。 |
