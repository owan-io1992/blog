...
| **`configMap`** | 將 [ConfigMap](/docs/k8s-doc-reading/k8s-doc-reading-configuration-configmaps/) 物件的內容以**檔案形式**掛載到容器中。 | 將 `nginx.conf` 設定檔注入到 NGINX 容器中。 |
| **`secret`** | 將 [Secret](/docs/k8s-doc-reading/k8s-doc-reading-configuration-secrets/) 物件的內容以**檔案形式**掛載到容器中。 | 將 TLS 憑證和私鑰掛載到 Ingress Controller 中。 |
